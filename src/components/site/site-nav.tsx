...
const RESOURCES: NavGroup = {
  ...
  items: [
    ...
    { label: "Self-host", href: "/self-host", desc: "Download & run it yourself" },
...
export function SiteNav({ floating = false }: { floating?: boolean }) {
  ...
  React.useEffect(() => {
    // rAF-batch so we update state at most once per frame, not once per scroll
    // event (which fires far more often and chops scrolling on phones).
    let ticking = false;
    const onScroll = () => {
      if (ticking) return;
      ticking = true;
