...
export function AiBuilder() {
  ...
  // One-time handoffs from elsewhere on the site:
  //  • construct:handoff  — a full blueprint (playground "Open in Construct")
  //    loads straight into the result view.
  //  • construct:describe — a prefilled description (the /quiz match) seeds the
  //    describe box so the user just hits "Generate".
  React.useEffect(() => {
    ...
    try {
      ...
      const desc = sessionStorage.getItem("construct:describe");
      if (desc) {
        sessionStorage.removeItem("construct:describe");
        setDescription(desc);
      }
    } catch {
